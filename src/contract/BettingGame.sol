...
contract BettingGame is VRFConsumerBase {
  ...
  // keyHash - one of the components from which the final random value will be generated by Chainlink VFRC.
  ...
  // !UPDATE
  // Returns the latest ETH/USD price from Chainlink oracles.
  ...
  // ETH in Wei (converted to compatibility with ethUsd (10**18 * 10**8)),
  // by ethUsd.
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    ...
    // Send the final random value to the verdict();
    verdict(randomResult);
  }

  // Send rewards to the winners.
  function verdict(uint256 random) public payable onlyVFRC {
    // Check bets from the latest betting round, one by one
    for (uint256 i = lastGameId; i < gameId; i++) {
      // Reset winAmount for the current user
      uint256 winAmount = 0;

      // If the user wins, they receive 2x their betting amount
    ...
    // Save the current gameId to lastGameId for the next betting round
    lastGameId = gameId;
  }
